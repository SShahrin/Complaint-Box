...
const [complaints, setComplaints] = useState([]); // শুরুতে খালি থাকবে
  ...
  const fetchComplaints = async () => {
    try {
      ...
      setComplaints(response.data); // ডাটাবেসের ডাটা এখানে জমা হবে
      setLoading(false);
    } catch (error) {
      console.error("Database theke data asche na:", error);
      setLoading(false);
    }
  };
...
  // ১. LocalStorage থেকে ডাটা লোড করা (রিফ্রেশ করলেও ডাটা থাকবে)
...
  };
  // Pie Chart এর জন্য ডাটা
  const pieData = [
  { name: 'Solved', value: adminData.resolvedCount },
  { name: 'In Progress', value: adminData.inProgressCount || 0 }, 
  { name: 'Pending', value: adminData.pendingCount },
  ];

const COLORS = ['#2dce89', '#feb019', '#f5365c']; // Green, Orange, Red
//   // Bar Graph এর জন্য ডামি ডাটা (আপনি চাইলে আপনার মতো পরিবর্তন করতে পারেন)
  const monthlyData = [
    { month: 'Jan', solved: 40, pending: 10 },
    { month: 'Feb', solved: 30, pending: 20 },
    { month: 'Mar', solved: 50, pending: 5 },
    { month: 'Apr', solved: 25, pending: 15 },
  ];

 

// const [complaints, setComplaints] = useState([
//   { id: 'CP-101', user: 'Sabbir', subject: 'Library Fan issue', priority: 'High', status: 'Pending', time: '2h ago', dept: 'Electrical' },
//   { id: 'CP-102', user: 'Mitu', subject: 'Wifi Down', priority: 'Urgent', status: 'In Progress', time: '10m ago', dept: 'IT' },
//   { id: 'CP-103', user: 'Rahat', subject: 'Water Leak', priority: 'Low', status: 'Resolved', time: 'Yesterday', dept: 'Plumbing' },
// ]);
...
const updateStatus = async (id, newStatus) => {
    try {
      // ব্যাকএন্ডে খবর পাঠানো যে স্ট্যাটাস বদলে দাও
      await axios.put(`http://localhost:5000/api/complaints/${id}`, { status: newStatus });
      
      // স্ক্রিনেও সাথে সাথে আপডেট করা
      setComplaints(prev => prev.map(c => 
        c.id === id ? { ...c, status: newStatus } : c
      ));
    } catch (error) {
      alert("Database-e update kora jachhe na!");
    }
...
useEffect(() => {
  ...
  const inProgress = complaints.filter(c => c.status === 'In Progress').length; // আলাদা ফিল্টার
...
  return (
    <>
      {/* ১. Stats Grid (উপরে থাকবে) */}
     {/* ১. Stats Grid - পুরোপুরি ডাইনামিক */}
<motion.div 
...
  
  {/* Total Complaints: adminData থেকে আসছে */}
...
  {/* Pending Issues: adminData.pendingCount থেকে আসছে */}
  <motion.div whileHover={{ scale: 1.05 }} className="admin-stat-card">
    <div>
      <p>Pending Issues</p>
...
  {/* Resolved Count: adminData.resolvedCount থেকে আসছে */}
  <motion.div whileHover={{ scale: 1.05 }} className="admin-stat-card">
    <div>
      <p>Total Resolved</p>
...
  {/* Resolution Rate: এটি একটি ক্যালকুলেটেড ডাইনামিক ভ্যালু */}
  <motion.div whileHover={{ scale: 1.05 }} className="admin-stat-card">
    <div>
      <p>Success Rate</p>
      <h3>
...
      {/* ২. Welcome & Rocket Cards (মাঝখানে থাকবে) */}
...
      {/* ৩. Graphs Section (সবার নিচে এবং জায়গা অদলবদল করা হয়েছে) */}
      <div className="admin-middle-grid">
        
        {/* Bar Graph - এখন বাম পাশে (Pie এর জায়গায়) */}
        <motion.div 
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          className="admin-info-card-neumorphic" 
          style={{ height: '350px' }}>
          <h4 style={{ marginBottom: '20px' }}>Monthly Reports</h4>
...
             <Bar dataKey="solved" fill="#2dce89" radius={[4, 4, 0, 0]} barSize={10} />
<Bar dataKey="inProgress" fill="#feb019" radius={[4, 4, 0, 0]} barSize={10} />
<Bar dataKey="pending" fill="#7928ca" radius={[4, 4, 0, 0]} barSize={10} />
            </BarChart>
          </ResponsiveContainer>
        </motion.div>

        {/* Pie Chart - এখন ডান পাশে (Bar এর জায়গায়) */}
        <motion.div 
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          className="admin-info-card-neumorphic" 
...
         {complaints.map(item => (
  <tr key={item.id}>
    <td><b>{item.id}</b><br/><small>{item.student_name}</small></td> {/* item.user ছিল */}
    <td>{item.topic}</td> {/* item.subject ছিল */}
    <td><span className={`status-pill ${item.status.replace(/\s+/g, '-').toLowerCase()}`}>{item.status}</span></td>
...
            className={`admin-info-card-neumorphic priority-border-${task.priority.toLowerCase()}`}
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <span className={`status-pill ${task.status.replace(/\s+/g, '-').toLowerCase()}`}>
                {task.status}
              </span>
...
           
    ...
    <div style={{ margin: '20px 0' }}>
      ...
      <h3 style={{ margin: '5px 0', fontSize: '18px' }}>{task.topic}</h3> {/* task.subject ছিল */}
      <p style={{ fontSize: '13px', color: '#525f7f' }}>Reported by: <b>{task.student_name}</b></p> {/* task.user ছিল */}
    </div>
...
              </select>
              <small style={{ color: '#8898aa' }}>{task.time}</small>
            </div>
          </motion.div>
        ))}
      </div>
  ...
  <tr key={solved.id}>
    <td>
      <div style={{ display: 'flex', flexDirection: 'column' }}>
        <span style={{ fontWeight: '600' }}>{solved.topic}</span> {/* solved.subject ছিল */}
        <small style={{ color: '#adb5bd' }}>ID: {solved.id} | By: {solved.student_name}</small> {/* solved.user ছিল */}
      </div>
    </td>
              <td><span className="tag-dept">Facility Admin</span></td>
              <td><small>{solved.time}</small></td>
              <td>
...
            {/* Add Staff Form (Only visible when showAddModal is true) */}
            {showAddModal && (
...
                      </select>
                    </td>
                    <td style={{ padding: '12px' }}><button style={{ color: 'red', border: 'none', background: 'none', cursor: 'pointer' }} onClick={() => setStaffList(staffList.filter(s => s.id !== staff.id))}>Remove</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
